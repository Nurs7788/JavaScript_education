...
function findIndexArr (filmToUser: string): number {
    return  movies.findIndex((movie)=>movie.filmName == filmToUser);
};

function add (filmToUser: string): void {
    if(findIndexArr(filmToUser) == -1) {   
        let filmToPush: MyMovie = {
        filmName: filmToUser,
        ratings: {}
    };
...
function del (filmToUser: string): void {
    if(findIndexArr(filmToUser) == -1) {
...
function rate (filmName: string, Array: MyMovie[]): void {
    ...
    if(indexArr == -1) {
        ...
        let userRate: string | null = prompt('Введите рейтинг от 1 до 10:');
        if(userRate === null) return;
        let userRates: number = parseInt(userRate);
...
function arrRatings (ratings: {[key: string]: number}): number[] {
    let arrayRatings: number[] = Object.values(ratings);
    return arrayRatings;
};

function list (Array: MyMovie[]): void {
    let ratingsSum: number = 0;
    let ratingsResult: number = 0;
...
function find (Array: MyMovie[], filmName: string): void {
    const indexArr = findIndexArr(filmName);
    ...
    if(indexArr == -1) {
        ...
        let filmSum: number = 0;
        let filmResult: number = 0;
