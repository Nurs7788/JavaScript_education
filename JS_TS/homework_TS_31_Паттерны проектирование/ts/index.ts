abstract class IAnimal {
    ...
    protected _happy: number;
    protected _randomState: number = 0;
    ...
    getAnimalAge(age: number): number {
...
interface IStrategy {
    valueUp: (value: number) => number;
    valueDown: (value: number) => number;
};
...
while(true) {
    let userToAnimal: string | null = prompt('Выберите вашего питомца: \nАкула - 1 \nЗебра - 2 \nПаук - 3');
    if(userToAnimal === null) break;

    let arrAnimals: IAnimal[] = [];

    if(userToAnimal === '1') {
        alert('Ваш питомец: Акула!');
        let nameAnimalToUser: string = prompt('Введите имя вашего питомца:') || '';
        let ageAnimalToUser: number = parseInt(prompt('Введите возраст вашего питомца:') || '');
        ...
        while(shark.hp >= 0 || shark.food >= 0 || shark.happy >= 0) {
            let toAction: string | null = prompt(`Что хотите сделать с ${shark.name}: \nИграть - 1\nКормить - 2\nЛечить - 3`);

            if(toAction === null) break;
        ...
        while(zebra.hp >= 0 || zebra.food >= 0 || zebra.happy >= 0) {
            let toAction: string | null = prompt(`Что хотите сделать с ${zebra.name}: \nИграть - 1\nКормить - 2\nЛечить - 3`);
        ...
        while(spider.hp >= 0 || spider.food >= 0 || spider.happy >= 0) {
            let toAction: string | null = prompt(`Что хотите сделать с ${spider.name}: \nИграть - 1\nКормить - 2\nЛечить - 3`);
